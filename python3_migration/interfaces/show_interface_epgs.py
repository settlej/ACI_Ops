...
def retrieve_leaf_list():
    ...
    url = """https://{apic}/api/node/mo/topology/pod-1.json?query-target=children&target-subtree-class=fabricNode&query-target-filter=and(wcard(fabricNode.id,"^1[0-9][0-9]"))""".format(apic=apic)
...
def gatherandstoreinterfacesforleaf(leaf):
    url = """https://{apic}/api/node/class/topology/pod-1/node-101/l1PhysIf.json""".format(apic=apic)
    result, totalcount = GetResponseData(url)
...
def main(import_apic,import_cookie):
    ...
    print(("\n1.) Physical interface\n" + 
            "2.) Port-channel/vPC\n"))
    int_type = custom_raw_input("What is the interface type?: ")
    if int_type == '1':
        ...
        print('\r')
        while True:
            leaf = custom_raw_input("What is leaf number?: ")
        ...
        while True:
            if fex == True:
                interface = custom_raw_input("What is the interface? (format: ethxxx/x/x): ")
            else:
                interface = custom_raw_input("What is the interface? (format: ethx/x): ")
            if interface in availableinterfaces:
                break
            else:
                print('\x1b[41;1mInvalid or interface does not exist...try again\x1b[0m\n')
        
        url = """https://{apic}/api/node/mo/topology/pod-1/node-{leaf}/sys/phys-[{interface}].json?rsp-subtree-include=full-deployment&target-node=all&target-path=l1EthIfToEPg""".format(leaf=leaf,interface=interface,apic=apic)
        #print(url)
        result, totalcount = GetResponseData(url)
        displayepgs(result)
    elif int_type == 2:
...
if __name__ == '__main__':
    while True:
        main()
        ask = custom_raw_input('Another interface? [y|n]:  ') or 'n'
        #print(ask)
        if ask[0].lower() == 'y':
            continue
        else:
