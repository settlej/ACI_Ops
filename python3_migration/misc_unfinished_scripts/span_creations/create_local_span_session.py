...
def POSTRequest(url, data, icookie):
    ...
    try:
        return urllib.request.urlopen(request, context=ssl._create_unverified_context()), None
    ...
    while True:
        try:
            ...
            if intsinglelist == 'invalid':
                continue
            return [x for x in finalsortedinterfacelist if x.number in intsinglelist], leaf
            #pdb.set_trace()
...
def create_span_dest_url(source_int, name, leaf):
    ...
    desturl = """https://localhost/api/node/mo/uni/infra/destgrp-{}.json""".format(spandestname)
...
def create_source_session_and_port(source_int, dest_int, name, leaf):
    ...
    sourceurl = """https://localhost/api/node/mo/uni/infra/srcgrp-{}.json""".format(spansessionname)
