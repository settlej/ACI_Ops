...
def getallroutes(leaf,vrf,tenant):
    get_Cookie()
    url = """https://localhost/api/node/mo/topology/pod-1/node-{}/sys/uribv4/dom-{}:{}.json?rsp-subtree=full&query-target=subtree&target-subtree-class=uribv4Route""".format(leaf,vrf,tenant)
...
#    url = """https://localhost/api//class/ipv4Addr.json?&query-target-filter==eq(ipv4Addr.addr,"{}")""".format(ip)
#    result, totalcount = GetResponseData(url)
#    for x in result:
#        if x != '':
...
def gathervzFilter():
    url = """https://localhost/api/node/class/vzFilter.json?rsp-subtree=full"""
...
def getcontractinfo(contract):
    ...
    url = """https://localhost/api/node/class/vzBrCP.json?query-target-filter=and(eq(vzBrCP.name,"{}"))&rsp-subtree=full""".format(contract)
    #print(url)
    result, totalamount = GetResponseData(url)
...
#    url = """https://localhost/api/class/vzBrCP.json?rsp-subtree=full"""
...
def getvzAnyContactusage():
    url = """https://localhost/api/class/vzAny.json?rsp-subtree=children&rsp-subtree-class=vzRsAnyToProv,vzRsAnyToCons"""
...
def getbdfromepg(tenant,app,epg):
    ...
    fvRsProv = []
    fvRsCons = []
    url = """https://localhost/api/node/mo/uni/tn-{tenant}/ap-{app}/epg-{epg}.json?query-target=children""".format(tenant=tenant,app=app,epg=epg)
    #print(url)
    result, totalamount = GetResponseData(url)
...
#def getfvRsCons():
#    url = """https://localhost/api/node/class/fvRsCons.json"""
#    result, totalamount = GetResponseData(url)
#    return result
#
#def getfvRsProv():
#    url = """https://localhost/api/node/class/fvRsProv.json"""
#    result, totalamount = GetResponseData(url)
#    return result

def getBDsubnetsandvrf(bd,tenant):
    url = """https://localhost/api/node/mo/uni/tn-{tenant}/BD-{bd}.json?query-target=children&target-subtree-class=fvRsCtx,fvSubnet""".format(bd=bd,tenant=tenant)
    get_Cookie()
    subnetlist = []
    result, totalamount = GetResponseData(url)
    #print(result)
...
def checkvzTaboo(tDn):
    url = """https://localhost/api/class/vzTaboo.json?rsp-subtree=full"""
    get_Cookie()
    result, totalamount = GetResponseData(url)
    #print(result)
...
def allips():
    get_Cookie()
    url = """https://localhost/api//class/ipv4Addr.json"""
    return GetResponseData(url)

def sourceEPGinfo():
    print('\n')
    tenant = eval(input("(EPG1) What tenant [default=SI] ")) or 'SI'
    app = eval(input("(EPG1) What App [default=APP-HQ] ")) or 'APP-HQ'
    epg = eval(input("(EPG1) What epg [default=EPG-VL9-SERVERS] ")) or 'EPG-VL9-SERVERS'
...
def destEPGinfo():
    print('\n')
    #print("\x1b[7")
    #tenant = raw_input("\x1b[1;152HWhat tenant [default=SI] ") or 'SI'
    tenant = eval(input("(EPG2) What tenant [default=SI] ")) or 'SI'
    app = eval(input("(EPG2) What App [default=APP-ISE] ")) or 'APP-ISE'
    epg = eval(input("(EPG2) What epg [default=EPG-VL10-ISE] ")) or 'EPG-VL10-ISE'
    #app = raw_input("\x1b[2;152Hwhat App [default=APP-HQ] ") or 'APP-HQ'
    #epg = raw_input("\x1b[3;152Hwhat epg [default=EPG-VL9-SERVERS] ") or 'EPG-VL9-SERVERS'
    bd,fvRsCons,fvRsProv = getbdfromepg(tenant,app,epg)
    subnetlist,vrf = getBDsubnetsandvrf(bd=bd,tenant=tenant)
    ...
    for contract in match:
        ...
        for vzsubject in contractobject.vzSubjectlist:
            ...
            for level in combinedlevels:
                for entry,priority in list(level.items()):
...
def iproutecheckfromleaf():
    cidrcollection = {}
    askleaf = eval(input("What leaf [default=101] ")) or 101
    asktenant = eval(input("what tenant [default=SI] ")) or 'SI'
    askvrf = eval(input("What vrf [default=SI]: ")) or 'SI'
    askipaddress = eval(input("Compair ip address to routing table: "))
    ...
    for route in routes:
        if ipaddress.ip_address(str(askipaddress)) in ipaddress.ip_network(str(route.routeprefix)):
