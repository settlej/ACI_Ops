...
class s_compVm():
    ...
    def __repr__(self):
        return self.dn
    def add_VNic_child(self, obj):
        self.VNic_children.append(obj)

class s_compVNic():
    def __init__(self, **kwargs):
        self.__dict__.update(kwargs)
        self.RsDlPol_children = []
        self.EpPConn_children = []
    def __repr__(self):
        return self.name
    def add_RsDlPol_child(self, obj):
        self.RsDlPol_children.append(obj)
    def add_EpPConn_child(self, obj):
        self.EpPConn_children.append(obj)
...
#    topcompVm = s_compVm(**result[0]['compVm']['attributes'])
#    if result['compVm'].get('children'):
#        for compVmchild in result[0]['compVm']['children']:
#            if compVmchild.get('compVNic'):
#                topcompVm.add_VNIC_child(s_compVNic(**compVmchild['compVNic']['attributes']))
#                if compVmchild['compVNic'].get('children'):
#                    for compVNicchild in compVmchild['compVNic']['children']:
#                        if compVNicchild.get('compRsDlPol'):
#                            topcompVm.VNic_children[-1].add_RsDlPol_child(s_compRsDlPol(**compVNicchild['compRsDlPol']['attributes']))
#                        elif compVNicchild.get('compEpPConn'):
#                            topcompVm.VNic_children[-1].add_EpPConn_child(s_compEpPConn(**compVNicchild['compEpPConn']['attributes']))
#    return topcompVm
...
def vm_search_function(vm_name):
    ...
    if totalcount == '0':
       ...
       # compVM = None
        ...
        k = [x for x in compVM.compVNiclist if x in  fvRsVmlist]
       # print(k)
       ...
       # print(compVM.compVNiclist)
        for vminterface in compVM.compVNiclist:
            if vminterface.mac == mac:
                ...
                print(("Helpful info:\n\tHost: {}\tvnic_status: {}\t\tvnic_ip: {}".format(result[0]['compHv']['attributes']['name'],vminterface.operSt ,vminterface.ip)))
