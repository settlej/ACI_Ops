...
def main():
    ...
    print(("\n1.) Physical interface\n" + 
            "2.) Port-channel/vPC\n"))
    int_type = eval(input("What is the interface type?: "))
    if int_type == '1':
        ...
        print('\r')
        while True:
            leaf = eval(input("What is leaf number?: "))
        ...
        while True:
            if fex == True:
                interface = eval(input("What is the interface? (format: ethxxx/x/x): "))
            else:
                interface = eval(input("What is the interface? (format: ethx/x): "))
...
if __name__ == '__main__':
    while True:
        main()
        ask = eval(input('Another interface? [y|n]:  ')) or 'n'
        #print(ask)
        if ask[0].lower() == 'y':
            continue
        else:
