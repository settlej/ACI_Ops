...
def displayfaultSummaryandSelection(orderedlist):
    ...
    print('\n')
    while True:
        selected = eval(input('Which fault?:  '))
...
def retrievePortChannelName(apic, cookie, PoNum, leaf):
    ...
    poresult = result[0][0]["pcRtAccBndlGrpToAggrIf"]["attributes"]['tDn']
    poposition = result[0][0]["pcRtAccBndlGrpToAggrIf"]["attributes"]['tDn'].rfind('accbundle-')
...
def askrefresh(detail_function, is_function):
    while True:
        refresh = eval(input("Would you like to refresh? [y/n]: "))
        if refresh.lower() == 'y' and refresh != '':
            result = is_function()
            if result == []:
                print("\nNo Faults Found (all faults are cleared)...\n")
                eval(input("#Press enter to refresh fault list..."))
                return 1
            else:
...
def askmoreDetail(apic):
    while True:
        ...
        print('\t')
        while True:
            userselected = eval(input("Which fault?: "))
...
def refreshloop(detail_function, fault, apic, cookie):
    ...
    while True:
        ...
        ask = eval(input('Would you like to refresh? [y=default|n]:  ')) or 'y'
    ...
    if os.path.isfile('/.aci/.sessions/.token'):
        ...
        while True:
            ...
            if unauthenticated:
                ...
                print(error)
                apic = eval(input("Enter IP address or FQDN of APIC: "))
                timedout = False
            else:
                print(error)
            ...
            except urllib.error.HTTPError as auth:
